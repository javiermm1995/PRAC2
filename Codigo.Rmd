---
title: PRAC2. Análisis de supervivencia de pasajeros en el Titanic
output:
  html_document:
    df_print: paged
    toc: true
...
Inspeccionamos los primeros registros para conocer su estructura
...
Se presenta el tipo de datos de cada columna y se realiza un análisis estadístico de los datos
...
Se eliminan las variables que no aportan información al análisis
...
Buscamos valores nulos y se reemplazan por la media
...
## Valores atípicos en número de familiares embarcados
...
## Valores atípicos en la variable Fare

Se visualiza el boxplot de la variable Fare.
...
Inicialmente se busca el número de valores únicos que existen en cada variable.

```{r}
# Primero, comprobamos qué variables son las que debemos binarizar
apply(df2, 2, function(x) length(unique(x)))
```
Se transforma la variable sex
...
Creación de la variable Fam. Esta variable indica si el pasajero tiene familiares a bordo del barco o no.
...
Factorización de las variables Survived, Pclass, Sex y Fam
...
## Estudio preliminar de variables
...
### Hipótesis 1. Cuando uno paga más por un billete de primera, puede pensar que paga no solo por lujo sino por un aumeno de la seguridad. Comprobamos si efectivamente existe una correlación entre el precio del billete (Fare) y la supervivencia (Survived)
...
### Hipótesis 2. Realizamos una regresión logística múltiple para ver de qué manera los rasgos identitarios de la persona afectan a sus probabilidades de supervivencia. Lo evaluamos con las variables: sex, age, pclass (proxy de la clase social) y Fam.
...
## Análisis mediante reglas de asociación
